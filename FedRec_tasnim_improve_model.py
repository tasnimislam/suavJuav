...
def train(snr_g, iid_g, dataset_setup_g):
    ...
    MC = 20 # Number of Monte Carlo trials
...
def main():
    ...
    ber = []
    for i in range(1, 16):
        with open(f'tanh_plot_ber_iid_{iid}_dataset_setup_{dataset_setup}.txt', 'w+') as f:
            f.write(str(i), str(train(i, iid, dataset_setup)), '\n')
        ber.append(train(i, iid, dataset_setup))
